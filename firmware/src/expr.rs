        ...
        match code[pc] {
            ...
            OP_SCALE => {
                ...
                binop(&mut stack, &mut sp, |root, pos| {
                    ...
                    // Clamp so scale() always produces a valid MIDI note,
                    // even when composed with further arithmetic.
                    note.min(127)
                });
            }
            OP_IF_GT if sp >= 4 => {
...
/// Returns `true` if the bytecode contains a `scale()` opcode.
...
/// Semitone intervals for each of the 7 diatonic modes,
/// ordered brightest to darkest.
