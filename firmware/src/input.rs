...
/// Simple quadrature rotary encoder using two GPIO pins (A and B) with pull-ups.
/// Tracks direction by comparing current and previous A/B state.
/// Maintains an internal CC value (0-127) that increments/decrements on each step.
struct EncoderState {
    prev_a: bool,
    prev_b: bool,
    cc: u8,
...
pub struct Encoders<const N: usize> {
    pins_a: [Input<'static>; N],
    pins_b: [Input<'static>; N],
    state: [EncoderState; N],
}
...
impl<const N: usize> Encoders<N> {
    pub fn new(pins_a: [Input<'static>; N], pins_b: [Input<'static>; N]) -> Self {
        let state: [EncoderState; N] = core::array::from_fn(|i| EncoderState {
            prev_a: pins_a[i].is_low(),
            prev_b: pins_b[i].is_low(),
            cc: 64,
        });
        Self {
            pins_a,
    ...
    /// Poll all encoders. Returns up to N change events.
    pub fn poll(&mut self) -> [Option<EncoderEvent>; N] {
        let mut events: [Option<EncoderEvent>; N] = [const { None }; N];
        for (i, event) in events.iter_mut().enumerate() {
            let a = self.pins_a[i].is_low();
            let b = self.pins_b[i].is_low();
            let s = &mut self.state[i];

            // Detect transition on A channel
            if a != s.prev_a {
                // Determine direction: if A and B are the same after
                // A changes, the encoder moved clockwise (increment).
                if a == b {
                    s.cc = s.cc.saturating_add(1).min(127);
                } else {
                    s.cc = s.cc.saturating_sub(1);
                }
                *event = Some(EncoderEvent {
                    index: i as u8,
                    cc: s.cc,
                });
            }
    ...
    /// Return the current CC value for the given encoder.
    pub fn current_cc(&self, index: usize) -> u8 {
        self.state[index].cc
    }
}
